...
client.once("ready", async () => {
    ...
    if (guild) {
        await guild.commands.create({
            ...
            options: [
                { name: "user", description: "User to verify", type: 6, required: true }
            ]
        });
        await guild.commands.create({
            name: "announcewl",
...
client.on("guildMemberAdd", async (member) => {
    ...
    try {
        ...
        if (usedInvite?.inviter) inviterTag = usedInvite.inviter.tag;
...
2️⃣ Go to <#${WL_CHANNEL_ID}> and type \`WL\`  
...
/* ================= INTERACTIONS (VERIFY + ANNOUNCEWL + BUTTON) ================= */
...
client.on("interactionCreate", async (interaction) => {
    try {
        ...
        if (interaction.isChatInputCommand()) {
            ...
            const hasPermission = staffMember.roles.cache.some(role => WL_STAFF_ROLE_IDS.includes(role.id));
            ...
            if (interaction.commandName === "verify") {
                ...
                try {
                    ...
                    const wlMessage = messages.find(msg => msg.author.id === targetUser.id && msg.content.toLowerCase().trim() === "wl");
...
1️⃣ Go to <#${WL_CHANNEL_ID}> (Whitelist Access)  
2️⃣ Type \`WL\`  
